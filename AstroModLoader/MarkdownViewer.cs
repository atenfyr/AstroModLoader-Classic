...
namespace AstroModLoader
{
    public partial class MarkdownViewer : Form
    {
        ...
        private void MarkdownViewer_Load(object sender, EventArgs e)
        {
            ...
            string stylingHTML = "<!DOCTYPE html><html><head><style>html{font-family:Arial,Helvetica,sans-serif;background-color:" + ColorTranslator.ToHtml(this.BackColor) + ";color:" + ColorTranslator.ToHtml(this.ForeColor) + "}a{color:" + ColorTranslator.ToHtml(AMLPalette.LinkColor) + "}</style></head>";
        ...
        private void browser1_Navigating(object sender, WebBrowserNavigatingEventArgs e)
        {
            ...
            {
                e.Cancel = true;
                AMLUtils.OpenURL(e.Url.ToString());
            }
